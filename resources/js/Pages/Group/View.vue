    ...
    <AuthenticatedLayout>
        <div class="max-w-[768px] mx-auto h-full ">
            ...
            <div class="group  relative bg-white p-4">
                ...
                <div class="flex">
                    <div class="flex items-center justify-center relative group/thumbnail -mt-[64px] ml-[48px] w-[128px] h-[128px]  rounded-full overflow-hidden ">
                        <img :src="thumbnailImageSrc || group.thumbnail_url || '/img/avatar_default.png'"
                             class="w-full h-full object-cover">
                        <button
                            v-if="isCurrentUserAdmin && !thumbnailImageSrc"
                                class="absolute bg-white/25 rounded-full flex items-center justify-center opacity-0 group-hover/thumbnail:opacity-100">
                            <CameraIcon class="w-8 h-8"/>

                            <input type="file" class="absolute left-0 bottom-0 right-0 opacity-0 cursor-pointer"
                                   @change="onThumbnailChange">
                        </button>
                        <div v-else-if="isCurrentUserAdmin" class="absolute items-center justify-center flex l gap-2">
                            <button @click="cancelThumbnailImg" class="w-7 h-7 flex items-center justify-center bg-red-500/80
                                text-white rounded-full">
                                <XMarkIcon class="w-5 h-5 "/>

                            </button>
                            <button @click="submitThumbnailImg" class="w-7 h-7 flex items-center justify-center bg-emerald-500/80
                                text-white rounded-full">
